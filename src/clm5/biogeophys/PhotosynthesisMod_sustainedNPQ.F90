...
!rl update to the method used by SCOPE
  ...
  call NPQ(x,Kn);       ! Call NPQ subroutine to calculate seasonal dependence of NPQ
...
subroutine NPQ(x,Kn)
   ...
   !
   !! DESCRIPTION:
   ! Seasonal varying calculation of non-photochemic rate constant
   ! based upon Porcar-Castell et al. (2011) 2008-2009 MONI-PAM
   ! measurements from Scots Pine Forest, Finland.

   ! !REVISION HISTORY:
   ! written by Brett Raczka 4/19/18
   ! coefficients updated 5/14/18
   ! 6/12/18  Using coefficients for kNPQs for NWR temperature adjustment
   ! Kn function updated;  kn= kNPQs(time)+kNPQr(x)
   ! 2/28/19  Uses Kolari method to adjust kNPQs(time)
   ...
   real(r8), intent(out) :: Kn  ! total NPQ rate constant
   real(r8):: julday            ! julian day
   real(r8):: coeff1            ! polynomial coefficent for kNPQs(julian day)
   real(r8):: coeff2            ! polynomial coefficent for kNPQs(julian day)
   real(r8):: coeff3            ! polynomial coefficient for kNPQs(julian day)
   real(r8):: kNPQs		! sustained NPQ rate constant; function of time of year
   real(r8):: kNPQr		! reversible NPQ rate constant; function of light saturation
   real(r8) :: Kno        ! fitted parameter for kNPQr(x)
   real(r8) :: Beta       ! fitted parameter for kNPQr(x)
   real(r8) :: Alpha      ! fitted parameter for kNPQr(x)
   
   
   ! Perform the case specific calculations here.
   ! 2/28/2019 Using kNPQs coefficient for Kolari method for NWR
   
   
   julday=get_curr_calday(0)      ! offset=0
   
   if (julday>=15_r8 .and. julday<=46_r8) then
   coeff1= 0_r8
   coeff2= 0.0284
   coeff3= 5.3186 

   else if (julday>46_r8 .and. julday<=74_r8) then
   coeff1= 0_r8
   coeff2= -0.0230
   coeff3= 7.6812

   else if (julday>74_r8 .and. julday<=105_r8) then
   coeff1= 0_r8
   coeff2= -0.0487
   coeff3= 9.5882

   else if (julday>105_r8 .and. julday<=135_r8) then
   coeff1= 0_r8
   coeff2= -0.0840
   coeff3= 13.2935
   
   else if (julday>135_r8 .and. julday<=166_r8) then
   coeff1= 0_r8
   coeff2= -0.0514
   coeff3= 8.8939
   
   else if (julday>166_r8 .and. julday<=196_r8) then
   coeff1= 0_r8
   coeff2= -0.0091
   coeff3= 1.8570

   else if (julday>196_r8 .and. julday<=227_r8) then
   coeff1= 0_r8
   coeff2= -0.0019
   coeff3= 0.4535

   else if (julday>227_r8 .and. julday<=258_r8) then
   coeff1= 0_r8
   coeff2= 0.0021
   coeff3= -0.4525

   else if (julday>258_r8 .and. julday<=288_r8) then
   coeff1= 0_r8
   coeff2= 0.0194
   coeff3= -4.9052

   else if (julday>288_r8 .and. julday<=319_r8) then
   coeff1= 0_r8
   coeff2= 0.0664
   coeff3= -18.4437
   
   else if (julday>319_r8 .and. julday<=349_r8) then
   coeff1= 0_r8
   coeff2= 0.1047
   coeff3= -30.6616

   else
   ! Includes jday 1:15 and 349:365
   coeff1= 0_r8
   coeff2= 0_r8
   coeff3= 5.85
 ...
 ! Sustainable NPQ rate coefficient
 kNPQs=coeff2 * julday + coeff3
 
 ! Reversible NPQ rate coefficient
 ! kNPQr coefficients based upon a single fit of SCOPE/vanderTol formulation
 
 Kno=2.582
 Beta=20470
 Alpha=1.043
 
 kNPQr=Kno*(1+Beta)*exp(log(x)*Alpha)/(Beta+exp(log(x)*Alpha))
 
 ! Total Kn = kNPQs+ kNPQr 
 Kn=kNPQs+kNPQr
   
   
 ! Troubleshooting output
  !        write (iulog,*) 'current julday value in NPQ subroutine:'
  !        write (iulog,*) julday
  !        write (iulog,*) 'current x value in NPQ subroutine:'
  !        write (iulog,*) x
  !	   write (iulog,*) 'coeff1 value in NPQ subroutine:'
  !        write (iulog,*) coeff1
  !        write (iulog,*) 'coeff2 value in NPQ subroutine:'
  !	   write (iulog,*) coeff2
  !	   write (iulog,*) 'coeff3 value in NPQ subroutine:'
  !	   write (iulog,*) coeff3
  !	   write (iulog,*) 'Kn value in NPQ subroutine:'
  !	   write (iulog,*) Kn
